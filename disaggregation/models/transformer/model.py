...
try:
    from disaggregation.models.transformer.components.layers import Encoder, EncoderLayer, ConvLayer
    from disaggregation.models.transformer.components.selfattention import FullAttention, AttentionLayer
    from disaggregation.models.transformer.components.encoding import PositionalEncoding
except:
...
class TransformerTimeSeries(nn.Module):
    ...
    def __init__(self,
        ...
        self.out_layer = nn.Linear(
            self.n_encoder_time_steps, self.output_vector_sz, bias=True)
...
def main():
    ...
    model = TransformerTimeSeries(device,
                                  n_encoder_time_steps=20,
                                  encoder_mask=False,
                                  output_vector_sz=1,
                                  encoder_vector_sz=1,
                                  iterative=False).to(device)
    src = torch.rand(60, 20, 1).to(device)
